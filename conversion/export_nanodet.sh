...
for SIZE in 320 416; do
    ...
    # GitHub release tag that can go stale and 404 -- same workaround as
    # export_yolo.sh/export_yolox.sh: pre-create the file so it's skipped.
    python3 -c "
import numpy as np
...
"
    #
    # This model's ShuffleNetV2 backbone uses GroupConvolution, which onnx2tf
    # can't represent in SavedModel by default (confirmed from actual run
    # output: "WARNING: ... GroupConvolution ... saved_model does not support
    # GroupConvolution. If saved_model is needed, specify
    # --disable_group_convolution to retransform the model.") -- that flag
    # converts GroupConvolution into standard convolutions during conversion,
    # which resolves it. (onnx2tf's own -odrqt/-oiqt direct-export flags looked
    # like an alternative that sidesteps SavedModel, but they internally
    # reload from a SavedModel path too, so they hit the exact same wall --
    # --disable_group_convolution is the fix, not a different code path.)
    #
    ...
    onnx2tf -i "$ONNX_FILE" -o "$OUT_DIR/${OUT_PREFIX}_${SIZE}_sm" -osd --disable_group_convolution

    # onnx2tf can fail without necessarily returning a nonzero exit code the
    # shell's `set -e` catches -- verify the actual SavedModel file exists
    # before trusting this step succeeded, or the quantization steps below
    # fail with a confusing, unrelated-looking error instead.
